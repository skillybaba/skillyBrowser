          ...
          child: ListView(children: [
            auth
                ? UserAccountsDrawerHeader(
                    decoration:
                        BoxDecoration(color: CupertinoColors.systemPurple),
                    accountName: Text(user.name ?? 'No Name'),
                    accountEmail: Text(user.number))
                                        ...
                                        return AlertDialog(
                                          ...
                                          actions: [
                                            IconButton(
                                              ...
                                              onPressed: () {
                                                Share.share(
                                                    """HOST-NAME : ${info[0] ?? this.name.text} 
ROOM-ID : ${roomid}
                        """);
                                              },
                                            ),
            ...
            FlatButton.icon(
                ...
                onPressed: () async {
                  ...
                  if (!await ConferenceService(
                          name: this.name.text ?? info[0],
                          number: info[1],
                          docid: info[2],
                          roomid: this.controller.text)
                      .hostMeeting('join'))
                    Alert(
                            context: context,
